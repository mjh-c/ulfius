...
//  unsigned int mhd_flags = MHD_USE_THREAD_PER_CONNECTION;
  ...
  if (u_instance->mhd_daemon == NULL) {
    ...
    if (key_pem != NULL && cert_pem != NULL) {
      ...
      mhd_ops[4].option = MHD_OPTION_HTTPS_MEM_KEY;
      mhd_ops[4].value = 0;
      mhd_ops[4].ptr_value = (void*)key_pem;

      mhd_ops[5].option = MHD_OPTION_HTTPS_MEM_CERT;
      mhd_ops[5].value = 0;
      mhd_ops[5].ptr_value = (void*)cert_pem;
      ...
      if (u_instance->timeout > 0) {
        mhd_ops[6].option = MHD_OPTION_CONNECTION_TIMEOUT;
        mhd_ops[6].value = u_instance->timeout;
        mhd_ops[6].ptr_value = NULL;

        mhd_ops[7].option = MHD_OPTION_END;
        mhd_ops[7].value = 0;
        mhd_ops[7].ptr_value = NULL;
      }
    } else if (u_instance->timeout > 0) {
      mhd_ops[4].option = MHD_OPTION_CONNECTION_TIMEOUT;
      mhd_ops[4].value = u_instance->timeout;
      mhd_ops[4].ptr_value = NULL;
